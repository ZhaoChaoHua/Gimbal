...
{
	rt_kprintf("[err]EEPROM I2C2 Timeout\n");
    return RT_ETIMEOUT;
}
...
//    Set_pinMode(PB10, PWM_OPEN_DRAIN);
//    Set_pinMode(PB11, PWM_OPEN_DRAIN);
  /*!< GPIO configuration */
...
//    return sEE_ReadBuffer((uint8_t *)buffer, (uint16_t)pos, (uint16_t *)&count);
//}
...
//    RT_ASSERT(buffer != RT_NULL);
//    
//    return sEE_WriteBuffer((uint8_t *)buffer, (uint16_t)pos, (uint16_t)count);
//}
...
// 确认EEPROM是否工作正常
rt_err_t check_eeprom()
{
    uint8_t buf[2] = {0};
    uint16_t num = 2;
    uint16_t addr = EEPROM_POS;
    uint16_t data = EEPROM_VERSION;
    sEE_ReadBuffer(buf,addr,(uint16_t *)&num);
    if((uint16_t)(buf[0]<<8 | buf[1])!=(uint16_t)data)
    {
        num = 2;
        buf[0] = (uint8_t)(data >> 8 & 0x00FF);
        buf[1] = (uint8_t)data;
        sEE_WriteBuffer(buf,addr,num);
        buf[0] = 0;
        buf[1] = 0;
        sEE_ReadBuffer(buf,addr,(uint16_t *)&num);
        if((uint16_t)(buf[0]<<8 | buf[1])!=(uint16_t)data)
...
void EE_Debug()
{
    ...
    if(ret == RT_EOK)
    {
        rt_kprintf("[test]EEPROM found\n");
    }
    else{
        rt_kprintf("[test]EEPROM not found\n");
    }
}

FINSH_FUNCTION_EXPORT(EE_Debug, Debug EEPROM.)
#endif
