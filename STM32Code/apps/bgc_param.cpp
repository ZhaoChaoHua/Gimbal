...
const AP_Param::Info Gimbal::var_info[] PROGMEM = {
    ...
    // PD_PIT_P PD_PIT_D PD_PIT_FILT_HZ
    // PD_RLL_P PD_RLL_D PD_RLL_FILT_HZ
    // PD_YAW_P PD_YAW_D PD_YAW_FILT_HZ
    GGROUP(pd_stabilize_roll,     "PD_RLL_", AC_PD),
    GGROUP(pd_stabilize_pitch,    "PD_PIT_", AC_PD),
    GGROUP(pd_stabilize_yaw,      "PD_YAW_", AC_PD),
    ...
    // ATT_M_P  ATT_M_I
    GOBJECT(madgwick, "ATT_", Madgwick),
    
    // can encoder center angle
    // CAN_CENTER_P  CAN_CENTER_R  CAN_CENTER_Y
    GOBJECT(can, "CAN_", CAN),
    
    // MPU6500 ACC Calibration data
    // MPU_ACC_F_X Y Z
    // MPU_ACC_T_X Y Z
    GOBJECT(mpu, "MPU_", MPU6500),
		
		// ekf param
		GOBJECT(ekf, "EKF_", DCM_EKF),
		
		// Direction Align
		GOBJECT(direction_align, "DIR_", DirectionAlign),
