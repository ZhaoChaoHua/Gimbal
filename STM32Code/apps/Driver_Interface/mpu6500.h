...
/*
  ...
  SPI speed
 */
#define MPU6500_LOW_BUS_SPEED				   1000*1000      // 1MHz，用于配置基本寄存器
#define MPU6500_HIGH_BUS_SPEED				   11*1000*1000   // 将会运行在10.4 MHz, MPU6500SPI极限
#define GYRO_INTERATIONS                       10000
...
public:
    ...
    AP_Float              accel_offs_x;
    AP_Float              accel_offs_y;
    AP_Float              accel_offs_z;

	  AP_Float              accel_T_x;
    AP_Float              accel_T_y;
    AP_Float              accel_T_z;

    AP_Int8               imu_rotation;
    ...
    // MPU6500初始化
    void mpu_init(uint8_t sample_rate_div,uint8_t low_pass_filter,  int id);
    void disable_I2C();
    void reSetMPU6500();
