...
//	angle_error_camera = mul_vec(&M, &angle_error);
////  angle_error_camera.x = (cz*cy-sz*sx*sy)
//	angle_error = angle_error_camera;
	scop_ex = angle_error.x*1000;
	scop_ey = angle_error.y*1000;
	scop_ey = angle_error.z*1000;
  ...
  speed_setpoint.z = _pd_angle_yaw.get_pd();             //cal output
	...
	motor_r = output.x = constrain_float(output.x, -AC_ATTITUDE_RATE_ROLL_CONTROLLER_OUT_MAX, AC_ATTITUDE_RATE_ROLL_CONTROLLER_OUT_MAX);  //roll speed output limit
	
	_pid_rate_pitch.set_input_filter_d(speed_error.y);     //set pitch speed pid input
	output.y = _pid_rate_pitch.get_pid();                  //cal output
	motor_p = output.y = constrain_float(output.y, -AC_ATTITUDE_RATE_PITCH_CONTROLLER_OUT_MAX, AC_ATTITUDE_RATE_PITCH_CONTROLLER_OUT_MAX);  //pitch speed output limit
...
Vector3f AC_AttitudeControl::rate_controller_run(Vector3f speed_input, Vector3f speed_feedback)
{
	Vector3f speed_error;
	speed_error = speed_input - speed_feedback;
