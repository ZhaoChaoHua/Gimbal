...
//	mz *= -1.0f;
	...
	if(beta_0_time > 0)
		...
		else
		{
			...
			float norm_range = 17;
			float beta_conj = beta*(1 - acc_norm / norm_range);//9.81f*beta/acc_norm;
			if(beta_conj < 0.0001) beta_conj = 0.0001;
			scop_beta_rt = beta_conj * 10000;
			qDot1 -= beta_conj * s0;
			qDot2 -= beta_conj * s1;
			qDot3 -= beta_conj * s2;
			qDot4 -= beta_conj * s3;
...
int scop_beta_time;
int scop_axe, scop_aye, scop_aze, scop_accnorm;
void Madgwick::MadgwickUpdate(float gx, float gy, float gz, float ax, float ay, float az) 
{
	...
	Vector3f acc_sensor, acc_earth;
	ChMatrix3 dcm;
	dcm.from_quat(q0, q1, q2, q3);
	acc_sensor(ax, ay, az);
	
	acc_earth = mul_vec(&dcm, &acc_sensor);
	acc_earth.z = acc_earth.z - 9.81f;
	acc_norm = 1.0f/invSqrt(acc_earth.x*acc_earth.x + acc_earth.y*acc_earth.y + acc_earth.z*acc_earth.z);
	axe = acc_earth.x;
	aye = acc_earth.y;
	aze = acc_earth.z;
	
	scop_axe = acc_earth.x * 100;
	scop_aye = acc_earth.y * 100;
	scop_aze = acc_earth.z * 100;
  scop_accnorm = acc_norm * 100;
