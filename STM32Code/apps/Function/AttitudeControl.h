...
#define AC_ATTITUDE_RATE_RP_CONTROLLER_OUT_MAX          100.0f
#define AC_ATTITUDE_RATE_PITCH_CONTROLLER_OUT_MAX       13.0f // body-frame rate controller maximum output (for roll-pitch axis)
#define AC_ATTITUDE_RATE_ROLL_CONTROLLER_OUT_MAX        20.0f 
#define AC_ATTITUDE_RATE_YAW_CONTROLLER_OUT_MAX         30.0f // body-frame rate controller maximum output (for yaw axis)
...
#define AC_ATTITUDE_2000HZ_DT                            0.0005f // delta time in seconds for 2000hz update rate
#define AC_ATTITUDE_1000HZ_DT                            0.0010f // delta time in seconds for 1000hz update rate

#define AC_ATTITUDE_CONTROL_RATE_BF_FF_DEFAULT          1       // body-frame rate feedforward enabled by default

class AC_AttitudeControl {
public:
	AC_AttitudeControl( Attitude& att,
                        AC_PD& pd_angle_roll, AC_PD& pd_angle_pitch, AC_PD& pd_angle_yaw,
                        AC_PID& pid_rate_roll, AC_PID& pid_rate_pitch, AC_PID& pid_rate_yaw
                        ) :
        _imu(att),
        _pd_angle_roll(pd_angle_roll),
        _pd_angle_pitch(pd_angle_pitch),
        _pd_angle_yaw(pd_angle_yaw),
        _pid_rate_roll(pid_rate_roll),
        _pid_rate_pitch(pid_rate_pitch),
        _pid_rate_yaw(pid_rate_yaw),
        _dt(AC_ATTITUDE_2000HZ_DT),
        _angle_boost(0),
    ...
    void save_accel_roll_max(float accel_roll_max) { _accel_roll_max = accel_roll_max ; }
    ...
    void save_accel_pitch_max(float accel_pitch_max) { _accel_pitch_max = accel_pitch_max; }
    ...
    void save_accel_yaw_max(float accel_yaw_max) { _accel_yaw_max = accel_yaw_max; }
    ...
    //
   Vector3f rate_controller_run(Vector3f speed_input, Vector3f speed_feedback);
    ...
    float max_angle_step_bf_roll() { return max_rate_step_bf_roll()/_pd_angle_roll.kP(); }
    ...
    float max_angle_step_bf_yaw() { return max_rate_step_bf_yaw()/_pd_angle_yaw.kP(); }
    ...
    void bf_feedforward_save(bool enable_or_disable) { _rate_bf_ff_enabled = enable_or_disable; }
    ...
    static const struct AP_Param::GroupInfo var_info[];
		...
		Vector3f serial_PID(Vector3f setpoint, Vector3f angle_feedback, Vector3f speed_feedback);
    
         // rate controller to motor
    float                 motor_p;
    float                 motor_r;
    float                 motor_y;
