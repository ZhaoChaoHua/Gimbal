...
const AP_Param::GroupInfo AC_PD::var_info[] PROGMEM = {
    ...
    AP_GROUPINFO("P",    0, AC_PD, _kp, 0.4),

    // @Param: D
    // @DisplayName: PID Derivative Gain
    // @Description: D Gain which produces an output that is proportional to the rate of change of the error
    AP_GROUPINFO("D",    1, AC_PD, _kd, 0.0),
    ...
    AP_GROUPINFO("FILT_HZ", 2, AC_PD, _filt_hz, AC_PID_FILT_HZ_DEFAULT),
	  ...
	  // @Param: LIMIT
    // @DisplayName: PD Output limit
    // @Description: 
    // @Unit: Hz
    AP_GROUPINFO("LIMIT", 3, AC_PD, _limit, 1.0f),

    AP_GROUPEND
};

// Constructor
AC_PD::AC_PD(float initial_p, float initial_d, float initial_filt_hz, float dt, float limit) :
    _dt(dt),
    _input(0.0f),
    _derivative(0.0f)
{
...
// Output limit 
float output_limit(float out, float limit)
{
	if(out > limit) return limit;
	else if(out < -limit) return -limit;
	else return out;
}

// Comput and limit output
float AC_PD::get_pd()
{
    return output_limit(get_p() + get_d(), _limit);
}


/// Overload the function call operator to permit easy initialisation
void AC_PD::operator() (float p, float i, float d, float imaxval, float input_filt_hz, float dt, float limit)
{
    _kp = p;
    _kd = d;
