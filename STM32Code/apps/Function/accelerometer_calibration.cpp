...
calibrate_return Accel_Cal::accel_calibration_worker(detect_orientation_return orientation, void* data)
{
	...
	rt_kprintf("[cal] %s side acc result: [%d.%d%d%d%d %d.%d%d%d%d %d.%d%d%d%d]\n", detect_orientation_str(orientation),
				     (int)(worker_data->accel_ref[orientation][0]),(int)(abs(worker_data->accel_ref[orientation][0])*10) % 10,(int)(abs(worker_data->accel_ref[orientation][0])*100) % 10, (int)(abs(worker_data->accel_ref[orientation][0])*1000) % 10, (int)(abs(worker_data->accel_ref[orientation][0])*10000) % 10,
				     (int)(worker_data->accel_ref[orientation][1]),(int)(abs(worker_data->accel_ref[orientation][1])*10) % 10,(int)(abs(worker_data->accel_ref[orientation][1])*100) % 10, (int)(abs(worker_data->accel_ref[orientation][1])*1000) % 10, (int)(abs(worker_data->accel_ref[orientation][1])*10000) % 10,
				     (int)(worker_data->accel_ref[orientation][2]),(int)(abs(worker_data->accel_ref[orientation][2])*10) % 10,(int)(abs(worker_data->accel_ref[orientation][2])*100) % 10, (int)(abs(worker_data->accel_ref[orientation][2])*1000) % 10, (int)(abs(worker_data->accel_ref[orientation][2])*10000) % 10);
...
calibrate_return Accel_Cal::do_accel_calibration_measurements(float (&accel_offs)[3], float (&accel_T)[3][3])
{
	...
	if (result == calibrate_return_ok)
    {
        ...
        result = calculate_calibration_values(worker_data.accel_ref, accel_T, accel_offs, CONSTANTS_ONE_G);
			  ...
			  result = least_square_calibration(worker_data.accel_ref, cal_param.gains, cal_param.offset, CONSTANTS_ONE_G);
...
calibrate_return Accel_Cal::read_accelerometer_avg(float (&accel_avg)[detect_orientation_side_count][3], unsigned orient, unsigned samples_num)
{
	...
	while (counts < samples_num)
    {
		...
		if (poll_ret == RT_EOK)
        {
            ...
            if (changed)
            {
                accel_sum[0] += _att.Accel_af.x;
                accel_sum[1] += _att.Accel_af.y;
                accel_sum[2] += _att.Accel_af.z;
...
enum detect_orientation_return Accel_Cal::detect_orientation(bool lenient_still_position)
{
	...
	while (true) 
    {
        ...
        //copter.thread_backend[0]
            accelerometer_m_s2[0] = _att.Accel_af.x;
            accelerometer_m_s2[1] = _att.Accel_af.y;
            accelerometer_m_s2[2] = _att.Accel_af.z;
            poll_ret = RT_EOK;
...
calibrate_return Accel_Cal::least_square_calibration(float (&accel_ref)[detect_orientation_side_count][3], 
			                        float (&accel_offs)[3], float (&accel_T)[3], float g)
{
//	float d[6*6];
	float d_pi[6*6];
	float i[6] = {1.0f,1.0f,1.0f,1.0f,1.0f,1.0f};
	float k;
//	for(int i = 0; i < detect_orientation_side_count; i++)
//	{
//		d[i*6+0] = accel_ref[i][0]*accel_ref[i][0];
//		d[i*6+1] = accel_ref[i][1]*accel_ref[i][1];
//		d[i*6+2] = accel_ref[i][2]*accel_ref[i][2];
//		d[i*6+3] = accel_ref[i][0]*2;
//		d[i*6+4] = accel_ref[i][1]*2;
//		d[i*6+5] = accel_ref[i][2]*2;
//	}
	
//	pinverse(d, d_pi);
	float *v = mat_mul_vec(d_pi, i, 6, 6);
	cal_param.offset[0] = v[3]/v[0];
	cal_param.offset[1] = v[4]/v[1];
	cal_param.offset[2] = v[5]/v[2];
	k = v[3]*v[3]/v[0] + v[4]*v[4]/v[1] + v[5]*v[5]/v[2] + 1;
	cal_param.gains[0] = g*sqrt(v[0]/k);
	cal_param.gains[1] = g*sqrt(v[1]/k);
	cal_param.gains[2] = g*sqrt(v[2]/k);
