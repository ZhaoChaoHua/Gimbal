...
{
    rt_uint32_t id  : 29;   // 标识符
    rt_uint32_t ide : 1;    // STD & EXT           0-STD  1-EXT
    rt_uint32_t rtr : 1;    // 远程                0-DATA 1-ROMAT
    rt_uint32_t mode : 1;   // IDmask & IDlist     0-MASK 1-LIST
    rt_uint32_t fifo : 1;   // 绑定FIFO
    rt_uint32_t fno;
    rt_uint32_t foff;       // 当前过滤器在FR1&FR2中的地址偏移(不适用于ID32MASK Filter)
    rt_uint32_t mask;       // 计算出的屏蔽位
    
    rt_int32_t hdr;         // 过滤器索引号
    // 回掉函数
...
struct rt_can_hdr
{
    rt_uint32_t connected;            // 该结构体是否连接至对应的过滤器
    rt_uint32_t msgs;                 // 链表中的节点数
    struct rt_can_filter_item filter; // 过滤器参数
    struct rt_list_node list;         // rt_can_msg_list 中 hdrlist的头节点，指向rt_can_msg_list
};
#endif
...
struct rt_can_device
{
    ...
    void *can_rx;      // can接收缓冲区
    void *can_tx;      // can发送缓冲区
};
...
struct rt_can_msg
{
    rt_uint32_t id  : 29;      // 索引号
    rt_uint32_t ide : 1;       // 标识符类型
    rt_uint32_t rtr : 1;       // 远程发送请求
    rt_uint32_t rsv : 1;       // (N/C)
    rt_uint32_t len : 8;       // 数据长度
    rt_uint32_t priv : 8;      // 优先级  发送邮箱号
    rt_uint32_t hdr : 8;       // 过滤器匹配索引
