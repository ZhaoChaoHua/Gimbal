...
static void DMA_Configuration(struct stm32_spi_bus * stm32_spi_bus,
...
{   
    ...
    DMA_RX_InitStructure.DMA_Channel = stm32_spi_bus->DMA_Channel_RX;
    DMA_RX_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&(stm32_spi_bus->SPI->DR));
    DMA_RX_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
    DMA_RX_InitStructure.DMA_BufferSize = (uint32_t)size;
    ...
    {
        DMA_RX_InitStructure.DMA_Memory0BaseAddr = (uint32_t) recv_addr;
        DMA_RX_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
    }
    else
    {
        DMA_RX_InitStructure.DMA_Memory0BaseAddr = (uint32_t) (&dummy);
        DMA_RX_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Disable;
    }
    DMA_Init(stm32_spi_bus->DMA_Stream_RX, &DMA_RX_InitStructure);
    DMA_Cmd(stm32_spi_bus->DMA_Stream_RX, ENABLE);

    /* Configure DMA_TX Stream */
    DMA_Cmd(stm32_spi_bus->DMA_Stream_TX, DISABLE);

    DMA_TX_InitStructure.DMA_Channel = stm32_spi_bus->DMA_Channel_TX;
    DMA_TX_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&(stm32_spi_bus->SPI->DR));
    DMA_TX_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
    DMA_TX_InitStructure.DMA_BufferSize = (uint32_t)size;
    ...
    {
        DMA_TX_InitStructure.DMA_Memory0BaseAddr = (uint32_t)send_addr;
        DMA_TX_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
    }
    else
    {
        dummy = 0xFF;
        DMA_TX_InitStructure.DMA_Memory0BaseAddr = (uint32_t)(&dummy);
        DMA_TX_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Disable;
    }
    DMA_Init(stm32_spi_bus->DMA_Stream_TX, &DMA_TX_InitStructure);
                          ...
                          struct rt_spi_configuration* configuration)
{
    ...
    // SPI TI Mode
    //SPI_TIModeCmd(stm32_spi_bus->SPI, ENABLE);
    //SPI_SSOutputCmd(stm32_spi_bus->SPI, ENABLE);
    /* Enable SPI_MASTER */
    SPI_Cmd(stm32_spi_bus->SPI, ENABLE);
    SPI_CalculateCRC(stm32_spi_bus->SPI, DISABLE);
#ifdef SPI_USE_DMA
    // DMA Config
    DMA_Configuration(stm32_spi_bus, RT_NULL, RT_NULL, 1);
#endif
    return RT_EOK;
};
