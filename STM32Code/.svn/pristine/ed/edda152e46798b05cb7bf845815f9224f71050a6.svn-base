...
//extern Blink blink_thread;
//extern Time_point time;
...
//// run a 9-state EKF used to calculate orientation main thread attitude
...
//{
//    // 获取定时器时间
//    imuSampleTime_ms = time.get_time(8);
//    dtIMU = delta_time;

//    // 初始化需要的变量和常理
//    if (!FiltInit) {
//        StartTime_ms =  imuSampleTime_ms;
//        newDataMag = false;   // 新的地磁数据
//        YawAligned = false;   // 航向轴对齐
//        state.quat[0] = 1.0f; // 四元数初始化
...
//        for (uint8_t i=0; i <= 2; i++) Cov[i][i] = sq(Sigma_angErr);  // 初始化协方差矩阵
//        for (uint8_t i=3; i <= 5; i++) Cov[i][i] = sq(Sigma_velNED);
//        for (uint8_t i=6; i <= 8; i++) Cov[i][i] = sq(Sigma_dAngBias);
//        FiltInit = true; // 初始化标志位
//        rt_kprintf("[init]SmallEKF Alignment Started\n");
//    }
...
//        alignHeading();
//        YawAligned = true;
//        rt_kprintf("[init]SmallEKF Alignment Completed\n");
//    }
...
//// 9 stat-EKF   detal-angle    detal-velocity     joint-angle
//// get small ekf status
//// state prediction
...
//	// init gravity vector with(0,0,1g)
//    // NED gravity vector m/s^2
//    const Vector3f gravityNED(0, 0, CONSTANTS_ONE_G);
...
//{
//    float run_time = time.get_time(8) - StartTime_ms;
