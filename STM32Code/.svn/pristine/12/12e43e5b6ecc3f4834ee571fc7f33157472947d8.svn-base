...
public:
    ...
    // Constructor for PID
    AC_PD(float initial_p, float initial_d, float initial_filt_hz, float dt, float limit);
    ...
    void operator() (float p, float i, float d, float imaxval, float input_filt_hz, float dt, float limit );
    ...
    void        filt_hz(const float v);
		void        Limit(const float v) { _limit = v;}
